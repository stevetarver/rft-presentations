...
const (
	...
	// Darksky.net provides a free API key for low volume use:
	// Doc: https://darksky.net/dev
	...
	// NewsApi.org aggregates many feeds, free API key to devels, open source, 15m delay.
	...
	newsUrl = "https://newsapi.org/v2/top-headlines?country=us&apiKey=4deee96f178748b6b0ba0a2987a6427c"
)
...
// text draws msg to canvas at specified location with specified style.
func text(x, y int, textSize int, textAlign string, msg string) {
	format := "fill:white;font-size:%dpt;font-family:Calibri;text-anchor:%s"
	style := fmt.Sprintf(format, textSize, textAlign)
...
func drawNews() {
	...
	// articles is a list of objects, we want the following from each:
	//  - source.name
	//  - title
